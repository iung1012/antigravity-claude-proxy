...
// Middleware — CORS restrito a origens localhost apenas
app.use(cors({
    origin: (origin, callback) => {
        if (!origin) return callback(null, true); // CLI/curl sem origin
        try {
    ...
    try {
        ...
        if (stream) {
            ...
            try {
                ...
                // Continue with the rest of the stream; abort if client disconnects
