...
export function mountWebUI(app, dirname, accountManager) {
    ...
    app.post('/api/config/password', (req, res) => {
        try {
            ...
            // If current password exists, verify old password (timing-safe)
            if (config.webuiPassword) {
                const expected = Buffer.from(config.webuiPassword);
                const provided = Buffer.from(oldPassword || '');
    ...
    app.get('/api/auth/url', async (req, res) => {
        try {
            ...
            // DoS protection: limita fluxos simultâneos a 5
            if (pendingOAuthFlows.size >= 5) {
                return res.status(429).json({ status: 'error', error: 'Too many pending OAuth flows. Try again in a moment.' });
            }
